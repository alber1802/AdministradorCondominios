...
class EditFactura extends EditRecord
{
    ...
    protected function handleRecordUpdate(Model $record, array $data): Model
    {
        ...
        $record->update($facturaData);

       
       // Si el estado es pagado, crear el pago correspondiente
        if ($data['estado'] === 'pagado' && isset($data['tipo_pago'])) {
            $this->createPago($record, $pagoData, $data);

            $recipient = $record->user;

            Notification::make()
            ->title("Nueva Factura Creada")
            ->body("Se ha creado una nueva factura con estado {$record->estado} para tu  departamento {$record->departamento->numero} por un monto de {$record->monto} y tipo {$record->tipo}.")
            ...
            ->actions([
                \Filament\Notifications\Actions\Action::make('ver')
                    ->label('Ver Factura')
                    ->color('info')
                    ->url("/storage/{$record->comprobante_pdf}")
                    ->button(),
            ])
            ->duration(5000)
            ...
            Notification::make()
            ->title("Nueva Factura Creada")
            ->body("Se ha creado una nueva factura con estado {$record->estado} para el  departamento {$record->departamento->numero} por un monto de {$record->monto} y tipo {$record->tipo}.")
            ->icon('heroicon-o-bell')
            ->color("primary")
            ->actions([
                \Filament\Notifications\Actions\Action::make('ver')
            ...
            Notification::make()
            ->title("Nueva Factura Creada")
            ->body("Se ha creado una nueva factura con pago pendiente para tu departamento {$record->departamento->numero} por un monto de {$record->monto} y tipo {$record->tipo}.")
            ->icon('heroicon-o-bell')
            ->color("success")
            ->actions([
                \Filament\Notifications\Actions\Action::make('ver')
                    ->label('Ver Factura')
                    ->color('info')
                    ->url("/intelliTower/facturas/{$record->id}")
                    ->button(),
            ])
            ->duration(5000)
            ->sendToDatabase($recipient);
