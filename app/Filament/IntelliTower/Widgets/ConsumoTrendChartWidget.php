...
class ConsumoTrendChartWidget extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        // Si el usuario no tiene departamentos asignados, retornar datos vacíos
        ...
        // Obtener tipos de consumo únicos (solo de los departamentos del usuario)
        $tiposConsumo = Consumo::whereIn('departamento_id', $departamentosIds)
            ->distinct('tipo')
            ->pluck('tipo')
            ->toArray();
        ...
        foreach ($tiposConsumo as $tipo) {
            ...
            for ($i = 5; $i >= 0; $i--) {
                ...
                // Calcular costo total por tipo y mes (solo de los departamentos del usuario)
                $costoTotal = Consumo::whereIn('departamento_id', $departamentosIds)
                    ->where('tipo', $tipo)
